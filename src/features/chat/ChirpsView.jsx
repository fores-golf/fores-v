    ...
    <div className="h-[100dvh] w-screen bg-[#090d16] text-white font-sans flex flex-col fixed inset-0 z-40 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 scrolling-touch">
        {chirps.map((chirp) => {
          ...
          if (chirp.isBot) {
            return (
              <div key={chirp.id} className="w-full py-1.5 px-4 bg-gradient-to-r from-amber-500/10 via-[#0f172a] to-transparent border-l-4 border-amber-500 rounded-r-xl my-2 flex items-start gap-2.5">
          ...
          return (
            <div key={chirp.id} className={`flex gap-3 items-start max-w-[88%] ${containsStats ? 'w-full max-w-sm' : ''}`}>
